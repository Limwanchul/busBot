...
public class BusSearchController {
	 ...
	 * 공공데이터포털 APIKEY
	 */
	@GetMapping("/tellegramBusArrival")
	public HashMap<String, Object> tellegramBusArrival (@PathParam(value = "data") String data) {
		...
		try {
			...
			// 노선번호목록조회 (버스 번호, 지역)
			if (data.indexOf("/") == -1) {
				List<BusRoute> busRouteList = pDataAcess.getBusRouteList(data, null);

				if (CollectionUtils.isEmpty(busRouteList)) {
					resultMap.put("result", false);
					resultMap.put("msg", data + "번 버스가 없습니다.");

					return resultMap;
				}

				resultMap.put("result", true);
				resultMap.put("busInfo", busRouteList);
			...
			// 경유정류소목록조회
			if ("1".equals(dataSplit[1])) {
				List<BusStation> busStationRouteList = pDataAcess.getBusRouteStationList(Integer.parseInt(dataSplit[0]), null);
				if (CollectionUtils.isEmpty(busStationRouteList)) {
					resultMap.put("result", false);
					resultMap.put("msg", "존재하지 않는 버스 입니다.");
				}

				resultMap.put("result", true);
				resultMap.put("busInfo", busStationRouteList);

			}

			if ("2".equals(dataSplit[1])) {
				List<BusArrival> busArrivalList = pDataAcess.getBusArrivalList(dataSplit[0]);
				ObjectMapper mapper = new ObjectMapper().enable(SerializationFeature.INDENT_OUTPUT);
				busRouteStationListJson = mapper.writeValueAsString(busArrivalList);
				Gson gson = new Gson();
				BusArrival[] busArrivalArray = gson.fromJson(busRouteStationListJson, BusArrival[].class);
